    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                showSuccessModal('Error', response.message || "Error adding product");
            }
        },
        error: function(xhr, status, error) {
            if (!checkAuthorization(xhr)) return;
            showSuccessModal('Error', 'Error adding product: ' + error);
        }
    }); 
}
            ...
            if (products && products.length > 0) {
                products.forEach(product => {
                    ...
                    <tr>
                        ...
                        <td>
                            <button class="btn bbtn btn-warning" onclick="editProduct(${product[0]})">
                                <i class="fas fa-edit"></i> Edit
                            </button>
                            <button class="btn bbtn btn-danger" onclick="deleteProduct(${product[0]})">
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                editModal.hide();
                showSuccessModal('Success', response.message, loadProducts);
            } else {
                showSuccessModal('Error', 'Error: ' + response.message);
            }
        },
        error: function(xhr, status, error) {
            showSuccessModal('Error', 'Error updating product: ' + error);
        }
    });
}
...
function deleteProduct(id) {
    showConfirmModal('Confirm Delete', 'Are you sure you want to delete this product?', function() {
        $.ajax({
            url: '/delete-product',
            type: 'POST',
            contentType: 'application/json',
            ...
            error: function(xhr, status, error) {
                showSuccessModal('Error', 'Error deleting product: ' + error);
            }
        });
    });
}
...
function logout() {
    showConfirmModal('Confirm Logout', 'Are you sure you want to logout?', function() {
        $.ajax({
            ...
            error: function(xhr, status, error) {
                showSuccessModal('Error', 'Error logging out: ' + error);
            }
        });
    });
}
    ...
    products.forEach(product => {
        tbody += `
            <tr>
                ...
                <td>₱${product[4].toLocaleString()}</td>
                <td>
                    <button onclick="editProduct(${product[0]})" class="btn bbtn btn-warning">
                        <i class="fas fa-edit"></i> Edit
                    </button>
                    <button onclick="deleteProduct(${product[0]})" class="btn bbtn btn-danger">
                        <i class="fas fa-trash"></i> Delete
                    </button>
                </td>
            </tr>
